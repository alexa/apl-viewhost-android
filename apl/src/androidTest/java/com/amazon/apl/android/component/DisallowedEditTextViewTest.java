...
public class DisallowedEditTextViewTest extends AbstractDocViewTest {
    ...
    @Test
    public void testView_click_on_disallowed_EditText_does_not_open_keyboard() {
        RootConfig rootConfig = RootConfig.create("EditText Test", "1.0");
    ...
    @Test
    public void testView_disallowed_EditText_occupies_some_space() {
        RootConfig rootConfig = RootConfig.create("EditText Test", "1.0");
        onView(withId(com.amazon.apl.android.test.R.id.apl))
                .perform(inflate(DOC, "", "", "{}", APLOptions.builder().build(), rootConfig))
                .check(hasRootContext());
        float expectedHeight = mTestContext.getRootContext().findComponentById("myEditText").getBounds().getHeight();
        float expectedSiblingTop = mTestContext.getRootContext().findComponentById("myPlainText").getBounds().getTop();
        ...
        onView(withId(com.amazon.apl.android.test.R.id.apl))
                .perform(inflate(DOC, "", "", "{}", APLOptions.builder().build(), rootConfig))
                .check(hasRootContext());
        float actualHeight = mTestContext.getRootContext().findComponentById("myEditText").getBounds().getHeight();
        float actualSiblingTop = mTestContext.getRootContext().findComponentById("myPlainText").getBounds().getTop();

        assertEquals(expectedHeight, actualHeight, 0.1);
        assertEquals(expectedSiblingTop, actualSiblingTop, 0.1);
        assertEquals(expectedHeight, expectedSiblingTop, 0.1);
    }
}
